...
mod tests {
    ...
    #[test]
    fn test_aes_gcm_256_key_from_array_trait() {
        let bytes = [0xCDu8; AesGcm256Key::SIZE];

        let key: AesGcm256Key = bytes.into();

        let expected = AesGcm256Key::from_bytes(bytes);

        assert_eq!(
            key, expected,
            "From<[u8; 32]> should preserve the key material"
        );
    }

    #[test]
    fn test_aes_gcm_256_key_from_array_trait_preserves_data() {
        let bytes = [
        ...
        assert_eq!(
            key, expected,
            "From<[u8; 32]> should preserve every byte of the key"
        );
    }
}
